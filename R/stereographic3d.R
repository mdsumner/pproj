#' Illustrate stereographic
#'
#' May need 'rgl::rglwidget()' if you don't have interactive graphics.
#'
#' @return nothing, side effect a 3D plot
#' @export
#'
#' @examples
#' do_stereographic()
do_stereographic <- function() {
  ll <- get_map(na.rm = TRUE)
  a <- 6370997
xyz <- llh2xyz(cbind(ll, 0), rad = a)

## project to Stereographic
prj <- "+proj=stere +lon_0=0 +lat_0=-90 +ellps=sphere"
pxy <- reproj::reproj_xy(ll, prj, source = "OGC:CRS84")

## these are the projected map points on the plane tangent
## to the south pole
pxyz <- cbind(pxy, a)

## plotting viewpoint for rgl
um <- structure(c(-0.765494763851166, 0.532267570495605, 0.361521005630493,
                  0, -0.59319019317627, -0.366132825613022, -0.716980457305908,
                  0, -0.249262124300003, -0.763299465179443, 0.596011281013489,
                  0, 0, 0, 0, 1), .Dim = c(4L, 4L))

## reduce the input map the south
maxlat <- -10
llsub <- ll[,2] < maxlat

## plot
bg3d(bg = "black")
plot3d(xyz, col = "dodgerblue", axes = FALSE)
points3d(pxyz[llsub, ], col = "#6AB787FF")
view3d(userMatrix = um, zoom = 0.5)

## rays from the projection point at the north pole
ptz2 <- cbind(reproj::reproj_xy(ll[llsub, ][sample(sum(llsub), 25), ], prj, source = "OGC:CRS84"), a)
for (i in seq_len(nrow(ptz2))) {
  lines3d(rbind(c(0, 0, -a), ptz2[i,,drop = FALSE]), color = "grey", lwd =1)
}

## we can't do this with the maps data
# ## polygons for fun, plot without lighting to hide triangulation facets
# ll <- get_map(na.rm  = FALSE)
# fishyfish <- ll[ll[,1] > 100 & ll[,1] < 160 & ll[,2] > -55 & ll[,2] < -5, ]
# #fishsoup <- lapply(fishyfish@polygons[[1]]@Polygons, function(x) x@coords)
# invisible(lapply(fishsoup, function(x) polygon3d(llh2xyz(cbind(x, 0), rad = a), col = "#3B5D9AFF", lit = FALSE)))

}
